...
## Тарифы и лимиты (актуальная сетка)

- **Free** — `0₽/мес`: 10 сообщений/мес, до 30 сек на одно голосовое, приоритет очереди `low`.
- **Basic** — `~299₽/мес`: 200 сообщений/мес, до 120 сек, приоритет `fast/normal`.
- **Pro** — `~699₽/мес`: безлимит (технически 1 000 000 сообщений/мес), до 600 сек, приоритет `high`.
- **Business** — `~1490₽/мес`: как Pro + бизнес-возможности, приоритет `high + business`.
...
```text
Выберите тариф:
• Free (~0₽/мес): 10 сообщений, до 30s, приоритет low. Базовый доступ.
• Basic (~299₽/мес): 200 сообщений, до 120s, приоритет fast/normal. Оптимально для регулярного использования.
• Pro (~699₽/мес): безлимит сообщений, до 600s, приоритет high. Безлимит и высокий приоритет.
• Business (~1490₽/мес): безлимит сообщений, до 600s, приоритет high + business. Как Pro + бизнес-возможности.
```

`/limits` (пример для Business):

```text
📊 Ваш тариф: Business (BUSINESS)
Лимиты: безлимит сообщений/мес, до 600s, приоритет очереди: high + business.
План: Как Pro + бизнес-возможности.
Цена: ~1490₽/мес.
```
